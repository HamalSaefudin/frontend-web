...
1. **NEVER write mock/dummy data** in services
   - ❌ `const mockUsers = [...]` + `setTimeout(() => resolve(mockData), 500)`
   - ✅ `apiClient.get('/api/v1/users')` directly

2. **API calls return AxiosResponse** — don't unwrap `.data.data`
   - Services return raw `apiClient` responses
   - Hooks unwrap via `fetchDataAsync`

3. **All services use `apiClient`** from `@/services/api-client`
   - Import: `import apiClient from '@/services/api-client'`
