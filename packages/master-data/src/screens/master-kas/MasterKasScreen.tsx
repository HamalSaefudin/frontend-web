import { Button, AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogTitle, Switch, DataTable, LoadingOverlay } from "@frontend/ui"
...
import { useQueryMasterKasList, useMutationCreateMasterKas, useMutationUpdateMasterKas, useMutationDeleteMasterKas, useMutationUpdateMasterKasStatus, useMutationExportMasterKas, type MasterKas, type MasterKasFilters } from "./hooks";
...
export function MasterKasScreen() {
  ...
  // Handlers
  const handleEdit = useCallback((kas: MasterKas) => {
    setEditingKas(kas);
    setFormOpen(true);
  }, []);

  const handleDelete = useCallback((kas: MasterKas) => {
    setDeleteTarget(kas);
  }, []);

  const handleStatusToggle = useCallback(async (id: string, status: boolean) => {
    try {
  ...
  const columns: ColumnDef<MasterKasRow>[] = useMemo(
    ...
    [statusMutation.isPending, handleStatusToggle, handleEdit, handleDelete],
  );
